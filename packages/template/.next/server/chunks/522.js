            ...
            return (0, a.getTracer)().trace(
              ...
              async () => {
                ...
                let h =
                  ...
                  R = O("revalidate"),
                  P = l(O("tags") || [], `fetch ${c.toString()}`);
                if (Array.isArray(P))
                  for (let e of (v.tags || (v.tags = []), P))
                ...
                "string" == typeof x &&
                  ...
                  ((h && "default" === x) ||
                    i.warn(
                      `fetch for ${b} on ${v.urlPathname} specified "cache: ${x}" and "revalidate: ${R}", only one should be specified.`,
                    ),
                  (x = void 0)),
                  "force-cache" === x
                    ? (R = !1)
                ...
                if (a && v.incrementalCache) {
                  B = await v.incrementalCache.lock(a);
                ...
                if (!a || !U) return G(!1, f).finally(B);
                {
                  v.pendingRevalidates ??= {};
                  let e = v.pendingRevalidates[a];
                  return e
                    ? (await e).clone()
                    : (v.pendingRevalidates[a] = G(!0, f).finally(async () => {
                        (v.pendingRevalidates ??= {}),
                          delete v.pendingRevalidates[a || ""],
                          await B();
                      }));
